...
$(window).addEvent( 'load', function(){
	...
	$$(".bgpatterns").each( function( dp, i ){
			var as = dp.getElements("a");
			as.each( function( item, i ){
				item.addEvent( 'click', function(){
					as.removeClass("active"); item.addClass("active");
					dp.getElement("input").value = item.id.replace(/\.\w+/,"");
				});	
				if( item.id.indexOf(dp.getElement("input").value+".") !=-1 ){
					item.addClass( "active" );
				}				
			} );
	} );
